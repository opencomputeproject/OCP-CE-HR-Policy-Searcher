...
### Added
- README documentation overhaul: added Features list entries for credential support, domain auto-generation, run reports, and cost tracking; added `add-domain`, `report`, and `help` commands to Usage section with examples; added Site Credentials configuration section documenting all 4 auth types; updated Configuration file tree and Project Structure tree to include new modules (`src/tools/`, `src/reporting/`, `src/crawler/auth.py`, `credentials.yaml.example`); updated Test Structure to list all 21 test files (774 tests); updated "Adding a New Domain" section to show `add-domain` quick path
- Paywalled site credential support: four authentication types (form, basic, cookie, header) for accessing login-gated policy sites. Credentials stored in `config/credentials.yaml` (gitignored) with a shipped `.example` template documenting all auth types. Form auth uses Playwright to fill login forms and persist session cookies in the browser context. HTTP Basic Auth and custom headers (e.g., API keys) applied per-request via `HttpFetcher`. Cookie injection supported for both Playwright and HTTP contexts. Credentials validated with Pydantic models; passwords use `SecretStr` to prevent accidental logging. `Authenticator` class coordinates credential application across fetchers with O(1) domain lookup
- `add-domain` CLI command: auto-generates domain YAML configuration from URL(s). Fetches the page (HTTP first, Playwright fallback), extracts title/language, detects JS requirements, determines region from TLD, suggests output file, and prints or writes a complete domain YAML entry. Supports `--dry-run` for preview, `--file` to override target file, `--name` and `--id` to override auto-detected values, and multiple `--url` flags. Multiple URLs on the same hostname are automatically merged into a single entry with combined start_paths. Query strings in URLs are preserved in start_paths. Output YAML matches hand-crafted file style: canonical field order, quoted strings, empty fields omitted, literal block notes. Pure generator functions in `src/tools/domain_generator.py` handle ID generation, region detection, and file suggestion for US federal/state `.gov`, `.gov.uk`, `.gouv.fr`, `.gv.at`, `.admin.ch`, `.europa.eu`, `.riksdagen.se`, `.retsinformation.dk`, `.go.jp`, `.go.kr`, `.gov.au`, `.gov.sg` domains
